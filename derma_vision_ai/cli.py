"""
Command-Line Interface for DermaVision-AI: Clinical & Dermoscopic Dual-Modality Fusion Agent.
"""
import argparse
import csv
import json
...
from .agents import DermaVisionCoordinator

coordinator = DermaVisionCoordinator()


def main(argv=None):
    parser = argparse.ArgumentParser(prog="dermatology-multimodal-dermoscopy", description="DermaVision-AI: Clinical & Dermoscopic Dual-Modality Fusion Agent")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  DERMAVISION-AI: CLINICAL & DERMOSCOPIC DUAL-MODALITY FUSION AGENT")
        print(f"  Domain: Dermatology AI | Standard: ISIC Dermoscopy Challenge Standards")
    ...
    if args.command == "serve":
        try:
            ...
            app = create_app()
            if app:
                print(f"Starting DermaVision-AI: Clinical & Dermoscopic Dual-Modality Fusion Agent on http://{args.host}:{args.port}")
